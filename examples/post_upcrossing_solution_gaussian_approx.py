"""
Plots the solution to the post-upcrossing density where only the Gaussian term is considered,
and erfc-term is discarded, and compare it to the true solution.
"""
...
t1 = 10.0
p1 = SimulationParameters(threshold=0.01, dt=0.1, I_e=0.1, num_procs=10000)
...
p2 = SimulationParameters(threshold=0.01, dt=0.1, I_e=0.1, num_procs=10000)
t2 = 200
...
soln_fn_xv_args = jax.vmap(
    ou_soln_xv_after_upcrossing_arguments, in_axes=(0, None, None, None, None, None, None),
                                           out_axes=(0, 0, None)
)

# multiple time steps for (T, N, 2) xv-values, (T,) b-values, (T,) b_dot-values, (T,) t-values
soln_fn_xv_args = jax.vmap(
        soln_fn_xv_args, in_axes=(0, None, None, None, None, 0, None)
)
...
q, quad, prefactor = soln_fn_xv_args(
    xv_grids,
    mu_xy[upcrossing_ind],
    s_xv[upcrossing_ind],
    b[upcrossing_ind],
...
q = q.reshape((len(t_vec), num_vals, num_vals))
quad = quad.reshape((len(t_vec), num_vals, num_vals))
