...
class SimulatorBase(ABC):
    ...
    def compute_propagators(self, t):
        ...
        B = np.array(
            [
                [-2.0 / self.p.tau_y, 0.0, 0.0],
                [1.0 / self.p.C, -1 / self.p.tau_tilde, 0.0],
                [0.0, 2 / self.p.C, -2 / self.p.tau_x],
            ]
        )
        cov_prop = expm(B * t)
        B_inv = np.linalg.solve(B, np.eye(3))
        cov_prop_const = B_inv.dot(np.eye(3) - cov_prop).dot(
            np.array([[self.p.sigma2_noise, 0.0, 0.0]]).T
        )
...
class ParticleSimulator(SimulatorBase):
    def __init__(self, z_0, u_0, params, stim=None, fix_x_threshold=False):
    ...
    def simulate(self, t):
        ...
        for _ in range(num_steps):
            ...
            self.upcrossings[i] = (self.z[i, :, 1] >= self.b[i]) & (
                self.z[i - 1, :, 1] < self.b[i - 1]
            )
            self.downcrossings[i] = (self.z[i, :, 1] < self.b[i]) & (
                self.z[i - 1, :, 1] >= self.b[i - 1]
            )
...
class MembranePotentialSimulator(SimulatorBase):
    ...
    def propagate(self):
        ...
        self.u[i] = self.u_prop * (self.u[i - 1] - self.p.E_L) + self.p.R * I * (
            1 - self.u_prop
        )
...
class MomentsSimulator(SimulatorBase):
    def __init__(self, mu_0, s_0, params):
    ...
    def simulate(self, t):
        ...
        num_steps = int(np.rint(t / self.p.dt))
        self.s = np.zeros(
            (num_steps + 1, 3), dtype=np.float64
        )  # analytical solution to second cumulants
        self.s[0] = self.s_0

        self.mu = np.zeros(
            (num_steps + 1, 2), dtype=np.float64
        )  # analytical solution to expectations
        self.mu[0] = self.mu_0
        self._step += 1
    ...
    def propagate(self):
        i = self._step
        self.s[i] = (
            self.cov_prop.dot(self.s[i - 1][:, None]) - self.cov_prop_const
        ).squeeze()
...
class Simulator:
    def __init__(
        self, params=None, crossing_times=None, mu0=None, cov0=None, u0=None, seed=1234
    ):
    ...
    def compute_n1(self, mu_0=None, s_0=None, u_0=None, return_dists=False):
        ...
        for t in self.crossing_times:
            mu, s, n1 = self.compute_prob_upcrossing(
                u_0, mu_0, s_0, t, return_dist=True
            )
            n1s.append(n1)
            mus.append(mu)
            ss.append(s)
    ...
    def simulate_n2(self, mu_0=None, s_0=None, u_0=None):
        ...
        for i in range(0, num_t - 1, 1):
            ...
            for j in range(i + 1, num_t, 1):
                mu_0 = np.array([mu_y_b, 0.0])
                s_0 = np.array([s_y_b, 0.0, 0.0])
                z_0 = self.sample_z(mu_0, s_0)
                sim_j = ParticleSimulator(z_0, 0.0, self.p)
    ...
    def compute_n2(self, mu_0=None, s_0=None, u_0=None):
        ...
        for i in range(0, num_t - 1, 1):
            ...
            mu_i = np.array([mu_y_b, 0.0])
            s_i = np.array([s_y_b, 0.0, 0.0])
            u_i = usim.u[t_ind]
